...
function prompt($question, $default = false, $marker = ': ') {
	...
	while (true) {
		...
		if (!empty($line) || $line === '0') return $line;
...
function menu($items, $default = false, $title = 'Choose an item') {
	...
	foreach ($map as $idx => $item) {
                if ($item === 'Return') {
                    line('  r. %s', (string)$item);
                } else if ($item === 'Quit') {
                    line('  q. %s', (string)$item);
                } else {
	...
	while (true) {
		...
		$line = input();
                ...
                if(!is_numeric($line) && in_array('Return', $map) && $line === 'r') {
                    return 'return';
                } else if(!is_numeric($line) && in_array('Quit', $map) && $line === 'q') {
                    return 'quit';
